    ...
    public static void main(String[] args) throws IOException {
        ...
        count = docs
                .flatMapToPair((document) -> {    // <-- MAP PHASE (R1)
                    ...
                    String[] tokens = document.split(" ");                              //divide document in tokens

                    HashMap<String, Long> counts = new HashMap<>();
                    ArrayList<Tuple2<String, Long>> pairs = new ArrayList<>();

                    for (String token : tokens) {                                             //insert tokens in map with value = count of the token
                        counts.put(token, 1L + counts.getOrDefault(token, 0L));
                    }
                    for (Map.Entry<String, Long> e : counts.entrySet()) {                     // move map to array of tuples
                        pairs.add(new Tuple2<>(e.getKey(), e.getValue()));
                    }
                    return pairs.iterator();
                })
                .reduceByKey((x, y) -> x+y);    // <-- REDUCE PHASE (R1)                        reduce by key, sum values
        ...
        count = docs
                .flatMapToPair((document) -> {    // <-- MAP PHASE (R1)
                    ...
                    for (Map.Entry<String, Long> e : counts.entrySet()) {
                        pairs.add(new Tuple2<>(randomGenerator.nextInt(K), new Tuple2<>(e.getKey(), e.getValue())));  //add random number to the tuples
                    }
                    return pairs.iterator();
                })
